...
#if AbstractObjectMapDebugLevel == 0
...
{
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::setMLValueToFull()", scope::medium),
                  2, AbstractObjectMapDebugLevel)
  SIGHT_VERB(dbg << "ml=" << ml->str() << endl, 2, AbstractObjectMapDebugLevel)
  ...
  if(!found) {
    ...
    SIGHT_VERB(dbg << str() << endl, 2, AbstractObjectMapDebugLevel)    
    modified = true;
  }
  return modified;
}
...
{
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::isEmpty()", scope::medium),
                  2, AbstractObjectMapDebugLevel)
...
bool AbstractObjectMap::insert(AbstractObjectPtr o, LatticePtr lattice) {
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::insert()", scope::medium),
                  2, AbstractObjectMapDebugLevel)
  SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
  ...
  if(!o->isLive(latPEdge, comp, analysis)) {
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      dbg << "<b>AbstractObjectMap::insert() WARNING: attempt to insert dead mapping "<<o->strp(latPEdge)<<" =&gt; "<<lattice->str()<<"<\b>"<<endl;
    SIGHT_VERB_FI()
    return false;
  }
  ...
  for(it = items.begin(); it != items.end(); i++) {
    AbstractObjectPtr keyElement = it->first;
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      dbg << "&nbsp;&nbsp;&nbsp;&nbsp;keyElement="<<keyElement->str("            ")
          <<" mustEqual(o, keyElement, latPEdge)="<<o->mustEqual(keyElement, latPEdge, comp, analysis)
          <<" insertDone="<<insertDone<<" mustEqualSeen="<<mustEqualSeen<<endl;
    SIGHT_VERB_FI()
      ...
      if(!it->second->equiv(lattice.get()))
      {
        SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
          dbg << "&nbsp;&nbsp;&nbsp;&nbsp;keyElement="<<keyElement->str("            ")
              <<" mustEqual(o, keyElement, latPEdge)="<<o->mustEqual(keyElement, latPEdge, comp, analysis)
              <<" insertDone="<<insertDone<<" mustEqualSeen="<<mustEqualSeen<<endl;
          dbg << "&nbsp;&nbsp;&nbsp;&nbsp;Removing i="<<i<<", inserting "<<o->strp(latPEdge, "        ")
              <<"=&gt;"<<lattice->str("        ")<<endl;
        SIGHT_VERB_FI()
...
LatticePtr AbstractObjectMap::get(AbstractObjectPtr abstractObjectPtr) {
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::get()", scope::medium),
                  2, AbstractObjectMapDebugLevel)
  SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
  ...
  for (list<MapElement>::iterator it = items.begin();
    ...
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      dbg << "&nbsp;&nbsp;&nbsp;&nbsp;keyElement(equal="<<eq<<")="<<keyElement->str("&nbsp;&nbsp;&nbsp;&nbsp;")<<endl;
    SIGHT_VERB_FI()
    ...
    if(eq) {
      ...
      if(abstractObjectPtr->mustEqual(keyElement, latPEdge, comp, analysis)) {
        SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
          dbg << "&nbsp;&nbsp;&nbsp;&nbsp;Stopping search since mustEqual, ret="<<ret->str("&nbsp;&nbsp;&nbsp;&nbsp;")<<endl;
        SIGHT_VERB_FI()
        break;
      }
...
Lattice* AbstractObjectMap::remapML(const std::set<MLMapping>& ml2ml, PartEdgePtr fromPEdge)
{
  ...
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::remapML", scope::medium),
                  2, AbstractObjectMapDebugLevel)
  ...
  SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
    ...
    for(std::set<MLMapping>::const_iterator m=ml2ml.begin(); m!=ml2ml.end(); m++) {
      ...
      if(!m->from->isLive(fromPEdge, comp, analysis) || (m->to && !m->to->isLive(latPEdge, comp, analysis)))
        dbg << "<b>AbstractObjectMap::remapML() WARNING: Skipping dead ml2ml mapping "
            <<m->from->strp(fromPEdge)<<"(live="<<m->from->isLive(fromPEdge, comp, analysis)
            <<") =&gt; "<<(m->to ? m->to->strp(latPEdge) : "NULL")<<"(live="
            <<(m->to ? m->to->isLive(latPEdge, comp, analysis) : -1)<<")"<<endl
  ...
  SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
    SIGHT_VERB_DECL(scope, ("ml2ml", scope::medium), 2, AbstractObjectMapDebugLevel)
    for(std::set<MLMapping>::const_iterator m=ml2ml.begin(); m!=ml2ml.end(); m++) {
      if(!m->from) continue;
  ...
  for( ; m != ml2ml.end(); ++m) {
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      SIGHT_VERB_DECL(scope, ("MLMapping", scope::medium), 2, AbstractObjectMapDebugLevel)
    ...
    if(m->to) {
      ...
      SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
        SIGHT_VERB_DECL(scope, ("newM::insert", scope::low), 2, AbstractObjectMapDebugLevel)
        dbg << "m-&gtto=" << (m->to? m->to->str() : "NULLMemLocObjectPtr") << endl;
...
bool AbstractObjectMap::meetUpdate(Lattice* thatL) {
  SIGHT_VERB_DECL(scope, ("AbstractObjectMap::meetUpdate()", scope::medium),
                  2, AbstractObjectMapDebugLevel)
  ...
  try {
    ...
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      ...
      { 
        SIGHT_VERB_DECL(scope, ("this", scope::medium), 2, AbstractObjectMapDebugLevel)
        dbg << str()<<endl; 
      }
      { 
        SIGHT_VERB_DECL(scope, ("that", scope::medium), 2, AbstractObjectMapDebugLevel)
          dbg << that->str()<<endl; 
      }
    SIGHT_VERB_FI()
    ...
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      SIGHT_VERB_DECL(scope, ("that->items", scope::medium), 2, AbstractObjectMapDebugLevel)
      for(list<MapElement>::iterator itThat=that->items.begin(); itThat!=that->items.end(); itThat++)
    ...
    for(list<MapElement>::iterator itThis=items.begin(); 
      ...
      SIGHT_VERB_DECL(scope, ("itThis", scope::medium), 2, AbstractObjectMapDebugLevel)
      SIGHT_VERB(dbg << "this: "<<itThis->first->str()<<" ==&gt; "<<itThis->second->str()<<endl,
                 2, AbstractObjectMapDebugLevel)
      ...
      for(list<MapElement>::iterator itThat=that->items.begin(); 
        ...
        SIGHT_VERB_DECL(scope, ("itThat", scope::medium), 2, AbstractObjectMapDebugLevel)
        SIGHT_VERB(dbg << "that: "<<itThat->first->str()<<" ==&gt; "<<itThat->second->str()<<endl,
                   2, AbstractObjectMapDebugLevel)
        ...
        if(itThis->first->equalSet(itThat->first, latPEdge, comp, analysis)) {
          ...
          *thatMEIt = true;
          ...
          SIGHT_VERB_DECL(scope, (txt()<<"Meeting", scope::medium), 2, AbstractObjectMapDebugLevel)
          ...
          // Update the lattice at *itThis to incorporate information at *itThat
          {
            ...
            SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
              SIGHT_VERB_DECL(scope, ("before", scope::low), 2, AbstractObjectMapDebugLevel)
              dbg << itThis->second->str()<<endl; 
            SIGHT_VERB_FI()

            modified = itThis->second->meetUpdate(itThat->second.get()) || modified;
            SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
              SIGHT_VERB_DECL(scope, ("after", scope::low), 2, AbstractObjectMapDebugLevel)
              dbg << itThis->second->str()<<endl; 
    ...
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      SIGHT_VERB_DECL(scope, ("thisMustEq2thatMustEq", scope::medium), 2, AbstractObjectMapDebugLevel)
    ...
    SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      SIGHT_VERB_DECL(scope, (txt() << "thatMustEq bool list", scope::medium), 2, AbstractObjectMapDebugLevel)
    ...
    { 
      SIGHT_VERB_DECL(scope, ("inserting that->this", scope::medium), 2, AbstractObjectMapDebugLevel)
    ...
    for(list<pair<list<MapElement>::iterator, pair<list<MapElement>::iterator, int> > >::iterator meIt=thisMustEq2thatMustEq.begin();
      ...
      SIGHT_VERB_DECL(scope, (txt()<<"mustEqual mapping "<<meIt->second.second<<": "<<(meIt->first)->first->str(), scope::medium),
                      2, AbstractObjectMapDebugLevel)
      SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
      ...
      SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
        SIGHT_VERB_DECL(scope, ("thatThisIndexCompare", scope::low), 2, AbstractObjectMapDebugLevel)
        dbg << "meIt->second.second=" << meIt->second.second << ", thatIdx=" << thatIdx << endl;
      SIGHT_VERB_FI()
      if(meIt->second.second >= thatIdx) {
        SIGHT_VERB_IF(2, AbstractObjectMapDebugLevel)
          SIGHT_VERB_DECL(scope, ("MapElementCompare", scope::low), 2, AbstractObjectMapDebugLevel)
        ...
        for(; thatIt!=meIt->second.first; thatIt++, thatIdx++, thatMEIt++) {
          ...
          if(!(*thatMEIt)) {
            SIGHT_VERB(dbg << "Inserting at meIt->first="<<(meIt->first)->first->str()
                       <<" mapping "<<thatIt->first->str()<<" ==&gt; "
                       <<thatIt->second->str()<<endl,
                       2, AbstractObjectMapDebugLevel)
    ...
    for(; thatIt!=that->items.end(); thatIt++) {
      SIGHT_VERB(dbg << "Pushing end "<<thatIt->first->str()<<" ==&gt; "
                 <<thatIt->second->str()<<endl,
                 2, AbstractObjectMapDebugLevel)
      // NOTE: we do not currently update the part field in the lattice thatIt->second
      //       to refer to this->latPEdge. Perhaps we should make a copy of it and update it.
