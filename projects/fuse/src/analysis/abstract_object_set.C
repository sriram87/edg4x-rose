...
#if AbstractObjectSetDebugLevel==0
#define DISABLE_SIGHT
#endif

//DEBUG_LEVEL(AbstractObjectSetDebugLevel, 0);
...
{
  assert(that);
  SIGHT_VERB_DECL(scope, (txt()<<"AbstractObjectSet::insert("<<that->str()<<")", scope::medium), 
                  1, AbstractObjectSetDebugLevel)
  
  //dbg << "this="<<str()<<endl;
  
  // Do not insert mappings for dead keys
  if(!that->isLive(latPEdge, comp, analysis)) { 
    SIGHT_VERB(dbg << "<b>AbstractObjectSet::insert() WARNING: attempt to insert dead element "
               <<that->strp(latPEdge)<<"<\b>"<<endl,
               1, AbstractObjectSetDebugLevel)
    return false;
  }
  ...
  if(!containsEqualSet(that)) {
    SIGHT_VERB_DECL(indent, (""), 1, AbstractObjectSetDebugLevel)
    SIGHT_VERB(dbg << "Object not contained. Adding..."<<endl, 1, AbstractObjectSetDebugLevel)
    // Inserting into"<<endl<<str("")<<endl;    
...
{
    SIGHT_VERB_DECL(scope, ("AbstractObjectSet::remove", scope::medium), 1, AbstractObjectSetDebugLevel)
    bool retval = false;
    bool found = false;
    assert(that);
    ...
    if(!found) {
        //try {
      SIGHT_VERB(dbg << "<b>AbstractObjectSet::remove()"<<endl, 1, AbstractObjectSetDebugLevel)
      SIGHT_VERB(dbg << "Cannot find "<<that->str("")<<endl, 1, AbstractObjectSetDebugLevel)
      SIGHT_VERB(dbg << str("&nbsp;&nbsp;&nbsp;&nbsp;")<<"</b>"<<endl, 1, AbstractObjectSetDebugLevel)
...
bool AbstractObjectSet::containsMay(const AbstractObjectPtr that)
{
  assert(that);
  bool retval = false;
  SIGHT_VERB_DECL(scope, ("AbstractObjectSet::containsMay()", scope::medium), 2, AbstractObjectSetDebugLevel)
  SIGHT_VERB(dbg << "that="<<that->str()<<endl, 2, AbstractObjectSetDebugLevel)
  SIGHT_VERB_DECL(indent,(" "), 2, AbstractObjectSetDebugLevel)
  std::list<AbstractObjectPtr>::iterator it = items.begin();
  for( ; it != items.end(); it++) {
    bool mayEqual = (*it)->mayEqual(that, latPEdge, comp, analysis);
    SIGHT_VERB(dbg << "it="<<(*it)->str("")<<") mayEqual="<<mayEqual<<endl, 2, AbstractObjectSetDebugLevel)
    if(mayEqual) {
      retval = true;
      break;
    } // end if
...
{
  SIGHT_VERB(dbg << "AbstractObjectSet::containsEqualSet("<<that->str("")<<")"<<endl, 1, AbstractObjectSetDebugLevel)
  assert(that);
  bool retval = false;
  std::list<AbstractObjectPtr>::iterator it = items.begin();
  for( ; it != items.end(); it++) {
...
bool AbstractObjectSet::isEmptyLat()
{
  ...
  for(std::list<AbstractObjectPtr>::iterator i=items.begin(); i!=items.end();) {
    ...
    SIGHT_VERB(dbg << "AbstractObjectSet::isEmpty() removing "<<(*i)->str()<<endl, 1, AbstractObjectSetDebugLevel)
    items.erase(i++);
  }
...
{
  SIGHT_VERB_DECL(scope, ("AbstractObjectSet::remapML", scope::medium), 1, AbstractObjectSetDebugLevel)
  SIGHT_VERB(dbg << "latPEdge="<<latPEdge->str()<<endl, 1, AbstractObjectSetDebugLevel)
  SIGHT_VERB(dbg << "getPartEdge()="<<getPartEdge()->str()<<endl, 1, AbstractObjectSetDebugLevel)
  ...
  for(std::list<AbstractObjectPtr>::iterator i=items.begin(); i!=items.end(); i++) {
    SIGHT_VERB_DECL(scope, (txt() << "item "<<i->get()->str(), scope::medium), 2, AbstractObjectSetDebugLevel)
    ...
    for(std::set<MLMapping>::const_iterator m=ml2ml.begin(); m!=ml2ml.end(); m++) {
      if(!m->from) continue;
      SIGHT_VERB_DECL(scope, (txt() << "m="<<m->str(), scope::low), 2, AbstractObjectSetDebugLevel)
      ...
      if((*i)->mustEqual(m->from, fromPEdge, comp, analysis)) {
        SIGHT_VERB(dbg << "mustEqual"<<endl, 1, AbstractObjectSetDebugLevel)
        ...
        SIGHT_VERB(dbg << "mayEqual"<<endl, 1, AbstractObjectSetDebugLevel)
        existsMayEqual = true;
        // Insert the corresponding value in ml2ml if it is not NULL
        vals2add.insert(m->to);
      }
    }
    SIGHT_VERB(dbg << "existsMustEqual="<<existsMustEqual<<" existsMayEqual="
               <<existsMayEqual<<endl, 2, AbstractObjectSetDebugLevel)
  ...
  SIGHT_VERB(dbg << "vals2add=" << endl, 2, AbstractObjectSetDebugLevel)
  for(set<MemLocObjectPtr>::iterator v=vals2add.begin(); v!=vals2add.end(); v++) {
    SIGHT_VERB_DECL(indent, (""), 1, AbstractObjectSetDebugLevel)
    SIGHT_VERB(dbg << (*v)->str("") << endl, 1, AbstractObjectSetDebugLevel)
...
{
  try {
    SIGHT_VERB_DECL(scope, (txt()<<"AbstractObjectSet::meetUpdate("<<(mode==may? "may": "must")<<")", scope::medium),
                    2, AbstractObjectSetDebugLevel)
    AbstractObjectSet *that = dynamic_cast <AbstractObjectSet*> (thatL);

    SIGHT_VERB_IF(2, AbstractObjectSetDebugLevel)
      { SIGHT_VERB_DECL(scope,("this", scope::low), 2, AbstractObjectSetDebugLevel);
        dbg << str()<<endl; }
      { SIGHT_VERB_DECL(scope, ("that", scope::low), 2, AbstractObjectSetDebugLevel)
        dbg << that->str()<<endl; }
    SIGHT_VERB_FI()
    ...
    bool modified = false;
    if(mode == may) {
      SIGHT_VERB(dbg << "latPEdge="<<latPEdge->str()<<endl, 2, AbstractObjectSetDebugLevel)
      SIGHT_VERB(dbg << "that->items("<<that->items.size()<<"="<<endl, 2, AbstractObjectSetDebugLevel)
      SIGHT_VERB_DECL(indent, (""), 2, AbstractObjectSetDebugLevel)
      for(std::list<AbstractObjectPtr>::iterator it=that->items.begin(); it!=that->items.end(); it++) {
        ...
        SIGHT_VERB(dbg << "it(live="<<itLive<<")="<<it->get()->str()<<endl, 2, AbstractObjectSetDebugLevel)
        
        // Do not copy over mappings with keys that are dead in this set's host part
        if(!itLive) continue;
        
        modified = insert(*it) || modified;
        SIGHT_VERB(dbg << "modified = "<<modified<<endl, 2, AbstractObjectSetDebugLevel)
      }
    } else if(mode==must) {
